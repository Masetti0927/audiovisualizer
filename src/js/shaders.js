...
export const fragmentShaderSimple = `
  uniform float u_red;
...
export const fragmentShaderPoints = `
  uniform float u_red;
  uniform float u_blue;
  uniform float u_green;
  void main() {
...
export const outerFragmentShader = `
  uniform float u_outerRed;
  uniform float u_outerGreen;
  uniform float u_outerBlue;
  void main() {
      float dist = length(gl_PointCoord - vec2(0.5));
      if (dist > 0.5) discard;
      gl_FragColor = vec4(vec3(u_outerRed, u_outerGreen, u_outerBlue), 1.0);
  }
`;

export const outerWireframeFragmentShader = `
  uniform float u_outerRed;
  uniform float u_outerGreen;
  uniform float u_outerBlue;
  void main() {
...
export const rayVertexShader = `
  ...
  void main() {
      ...
      if (a_isTip > 0.5) {
          ...
          float rayLen = displacement * u_rayLength * 10.0 * activated;
          pos += a_rayDir * rayLen;
      }
...
export const rayFragmentShader = `
  ...
  uniform float u_outerBlue;
  void main() {
      gl_FragColor = vec4(u_outerRed, u_outerGreen, u_outerBlue, 1.0);
  }
`;
